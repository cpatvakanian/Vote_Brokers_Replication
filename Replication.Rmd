...
Timothy Frye, Reuter and Szakony (2019) examine the voter behavior Russia and Venezuela and find different types of brokers, appeals, and targets have different effects on voter turnout. I successfully replicated all of their results. As a robustness test, I impute missing values in the dataset and find results in line with that of the originally study, but of a smaller magnitude. These results confirm the authors' original findings and suggest that the missing values in their sampled population do not bias the results.
...
testing <-felm(perclosejob100~ + citysize +  male + logage + polinterest + supervisor + lengthwork + coworker_weekly + edu+ gov|factor(strategy)|0|regionid, data=subset(rus, employed==1))

#summary(testing)

testing2 <-glm(is.na(perclosejob100)~ + citysize +  male + logage + polinterest + supervisor + lengthwork + coworker_weekly + edu+ gov|factor(strategy)|0|regionid, data=subset(rus, employed==1), family= binomial)
...
rus_emp2 <- rus_emp %>%
  filter(fulltreatment %in% c("employer_threat", "employer_gift", "employer_benign", "employer_org"))


rus_emp3 <- rus_emp %>%
  filter(is.na(perclosejob100))
```
